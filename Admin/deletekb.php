...
if (isset($_POST['submitdelete'])) 
{
  ...
  if ($result->num_rows > 0) 
  {
    while($row = $result->fetch_assoc()) 
    {
    	if($id == $row["id"])
      {
        $query = "DELETE FROM kbclothes WHERE id = '$id'";
        if ($connection->query($query) === TRUE)
        {
          echo"<script>alert('Record DELETED...');location='deletekb.php'</script>";
          break;
        }
      }
    }
  }
         echo"<script>alert('Record Not Deleted...');location='deletekb.php'</script>";  
