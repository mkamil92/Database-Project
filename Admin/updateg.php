...
if (isset($_POST['add'])) 
{
  ...
  if ($result->num_rows > 0) 
  {
    while($row = $result->fetch_assoc()) 
    {
      if($idcheck == $row["id"])
       {
          $query = "UPDATE gadgets SET id='$id', name='$name', price='$price', brand='$brand', description='$description', specification='$specification', photo='$photo' WHERE id = '$idcheck'";
          $query_run = mysqli_query($connection, $query);
          if($query_run)
          {
            echo"<script>alert('Record Updated...');location='updateg.php'</script>";
            break;  
          }
         ...
         echo"<script>alert('Record Not Updated...');location='updateg.php'</script>";  
