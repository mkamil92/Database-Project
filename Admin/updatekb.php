...
<h3 style="text-align: center;color: red; background-color: white;">Kids & Babies</h3>
<br>
...
if (isset($_POST['add'])) 
{
   ...
   // echo"<script>alert('checked');</script>"; 
    ...
    $query = "SELECT id FROM kbclothes";
    $result = $connection->query($query);
    $idcheck = $_POST['check'];


  $sql = "SELECT id FROM kbclothes";
  ...
  if ($result->num_rows > 0) 
  {
    while($row = $result->fetch_assoc()) 
    {
      if($idcheck == $row["id"])
       {
          $query = "UPDATE kbclothes SET id='$id', name='$name', price='$price', brand='$brand', description='$description', specification='$specification', photo='$photo' WHERE id = '$idcheck'";
          $query_run = mysqli_query($connection, $query);
          if($query_run)
          {
            echo"<script>alert('Record Updated...');location='updatekb.php'</script>";
            break;  
          }
         ...
         echo"<script>alert('Record Not Updated...');location='updatekb.php'</script>";  
